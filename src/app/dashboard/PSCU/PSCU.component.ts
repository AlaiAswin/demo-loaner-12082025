...
@Component({
  selector: 'app-PSCU',
  templateUrl: './PSCU.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Pscucomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR PSCU', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'There shall be no damaged, deformed or dirty appearance on exterior.\nLabels and markings attached on the main unit shall be completely legible .\nThe connector on the rear panel shall be normal', `${s1}`],
              ['2', 'Exterior of PSCU', 'Inspect the exterior of the PSCU for noticeable scratches, grime, and/or deformation that may affect the functionality.', `${s2}`],
              ['3', 'Start up Check', 'Press the Power switch od PSCU. Inspect the following item:\nThe Power switch on the front panel illuminates in green.\nPress the Power switch on the front panel.\nAfter startup, Check that there is no abnormal noise or vibration.\nAfter startup, check that the "BACKWARD" indicator on the force guage ( MAJ-2179) blinks .Press the power switch of PSCU again and check that PSCU stops.', `${s3}`],
              ['4', 'Endoscope image display/ rotation control check', 'Check that image displayed on the monitor ( OEV262H) are free of noise or blurred characters.\nDepress the FORWARD pedal of the foot switch ( MAJ-2181) , the spiral tube shall start rotating.\nDepress the BACKWARD pedal of the foot switch ( MAJ-2181) , the spiral tube shall start rotating.\nWhen you depress both the pedals of the foot switch ( MAJ-2181) simultaneously during to rotate the spiral tube, it shall stop rotating.', `${s4}`],
      ...
      if (image_data_url) {
        content.unshift(
          { text: 'Issue date :30 Dec 2019', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 00', alignment: 'right', fontSize: 5 },
          {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'PSCU_' + sno}`; // Custom file name
