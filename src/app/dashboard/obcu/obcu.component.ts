...
@Component({
  selector: 'app-obcu',
  templateUrl: './obcu.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class ObcuComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OBCU', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Clean and disinfect the OBCU according to the instructions in the corresponding Instructions Manual.', `${s1}`],
              ['2', 'Power on', 'Switch on the power supply and confirm that the OBCU ON.\nThe power LEDs illuminate', `${s2}`],
              ['3', 'Cooling FAN', 'Sending forth direction is correct', `${s3}`],
              ['4', 'Pause/Release function check', 'Pressure variation after leaving for one minute should be within 1 kPa \nPerform release and confirm that LED display is correct', `${s4}`],
              ['5', 'Inflate & Deflate function check', 'Verify and check the inflate & deflate function working ok.', `${s5}`],
              ['6', 'Outer appearance', 'No major scratch, dirt, or screw not tightened is present. Ensure the label and marking on the main body can be clearly read. ', `${s6}`],
              ['7', 'power off', 'Switch off the unit, disconnect all the acessories.\nEnsure the power switch is off', `${s7}`]
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OBCU_' + sno}`; // Custom file name
