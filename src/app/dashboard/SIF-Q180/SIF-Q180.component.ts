...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-SIF-Q180',
  templateUrl: './SIF-Q180.component.html',
  styleUrls: ['./SIF-Q180.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Sifq180Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR SIF-Q180', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'SIF-Q180_' + sno}`; // Custom file name
