...
@Component({
  selector: 'app-URF-V2R',
  templateUrl: './URF-V2R.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Urfv2rComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR URF-V2R', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Insertion section', 'Inspection of the insertion tube’s lock/free mechanism', `${s4}`],
                ['5', ' Image system', 'Inspection of the endoscopic image', `${s5}`],

                ['6', ' Control section;\nSwitch section', 'Inspection of the remote switches', `${s6}`],
                ['7', 'Connection to ancillaryequipment', 'Connection to the light source and video system center', `${s7}`],

                ['8', 'Instrument channelsystem', 'Inspection of the instrument channel', `${s8}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'URF-V2R_' + sno}`; // Custom file name
