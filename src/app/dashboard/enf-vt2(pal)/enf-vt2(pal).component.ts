...
@Component({
  selector: 'app-enf-vt2(pal)',
  templateUrl: './enf-vt2(pal).component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Enfvt2palComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR ENF-VT2(PAL)', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['3.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the angulation mechanism', `${s3b}`],
                ['4', 'Connection to ancillary equipment', 'Connection to the light source and video system center', `${s4}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'ENF-VT2(PAL)_' + sno}`; // Custom file name
