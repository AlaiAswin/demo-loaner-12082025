...
@Component({
  selector: 'app-NEUROSCOPES',
  templateUrl: './NEUROSCOPES.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Neuroscopescomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - NEUROSCOPES', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        { text: 'VISUAL INSPECTION OF THE ELECTRO SURGICAL GENERATOR AND ACCESSORIES', fontSize: 12 },
        {
          table: {
            ...
            body: [
              ...
              ['4', 'EYE PIECE SHROUD ADHESSIVE ', 'PROCEDURE PASSED', `${s4}`],
              ['5', 'DISTAL END- DAMAGE/ABNORMALITIES', 'NONE PESENT', `${s5}`],
              ['6', 'OUTER TUBE- DAMAGE/ABNORMALITIES', ' NONE PESENT', `${s6}`],
...

        {
          table: {
            ...
            body: [
              ...
              ['7', 'LIGHT TRANSMISSION', ' PROCEDURE PASSED', `${s7}`],
              ['8', 'IMAGE QULITY', ' PROCEDURE PASSED', `${s8}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'NEUROSCOPES_' + sno}`; // Custom file name
