...
@Component({
  selector: 'app-PROBE_DRIVING_UNIT',
  templateUrl: './PROBE_DRIVING_UNIT.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Probedrivingunitcomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR PROBE DRIVING UNIT', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
...

        {
          table: {
            ...
            body: [
              ...
              ['2', 'CONNECTOR', 'PROCEDURE PASSED', `${s2}`],
              ['3', 'CABLE ASSEMBLY', 'PROCEDURE PASSED', `${s3}`],
              ['4', 'APPEARANCE', 'PROCEDURE PASSED', `${s4}`],
              ['5', 'ULTRASOUND IMAGE', 'PROCEDURE PASSED', `${s5}`],
              ['6', 'WARRANTY STICKER', 'PROCEDURE PASSED', `${s6}`],
