...
@Component({
  selector: 'app-tjf-q180v',
  templateUrl: './tjf-q180v.component.html',
  styleUrls: ['./tjf-q180v.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})

export class TjfQ180vComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder, private popupservice: PopUpService, private router: Router, private renderer: Renderer2, private location: Location) {
    this.formGroup = this.fb.group({
      ...
      s4a_result: [, Validators.required],
      s4b_result: [, Validators.required],
      ...
      s15a_result: [, Validators.required],
      s15b_result: [, Validators.required],
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        let s4a = data['s4a_result'];
        let s4b = data['s4b_result'];
        ...
        let s15a = data['s15a_result'];
        let s15b = data['s15b_result'];
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15a == p) && (s15b == p) && (s16 == p) && (s17 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR TJF-Q180V', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4.1', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1.Inspection for smooth operation', `${s4a}`],
                ['4.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2.Inspection of the UP/DOWN/RIGHT/LEFT angulation mechanism', `${s4b}`],
                ['5', 'Control section;\nElevator section', 'Inspection of the forceps elevator mechanism:\nInspection for smooth operation', `${s5}`],
                ...
                ['11', 'Endoscope connector', 'Connection to the videoscope cable', `${s11}`],
                ['12', 'Image system', 'Inspection of the endoscopic image', `${s12}`],
                ['13', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s13}`],
                ['14', 'Air supply system', 'Inspection of the air-feeding function', `${s14}`],
                ['15.1', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function', `${s15a}`],
                ['15.2', 'Water supply system', 'Inspection of the objective lens cleaning function:\n2. Inspection of removing the remaining water from the objective lens', `${s15b}`],
                ['16', 'Suction System', 'Inspection of the suction function', `${s16}`],
                ['17', 'Instrument channel system', 'Inspection of the instrument channel and forceps elevator', `${s17}`],
              ],
            }
          }, function () {
            if (!image_data_url) {
          ...
          }(),
          {
            table: {
              widths: [125, 125, 125, 125],
              body: [
                [{ text: 'INSPECTION DATE', fillColor: '#CCCCCC', alignment: 'center' }, { text: `${this.startDate}` }, { text: 'INSPECTION BY', fillColor: '#CCCCCC' }, { text: `${data['insBy']}`, alignment: 'center', margin: [0, 30, 0, 0] }],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'TJF-Q180V_' + sno}`; // Custom file name
