...
@Component({
  selector: 'app-enf-xp',
  templateUrl: './enf-xp.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class EnfxpComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4 == p) && (s5 == p)) {
        console.log('Pass')
        overRes = 'Pass'
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR ENF-XP', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'ENF-XP_' + sno}`; // Custom file name
