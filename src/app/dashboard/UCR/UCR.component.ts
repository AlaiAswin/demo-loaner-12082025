  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR UCR', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the UCR must be performed according to the Instructions Manual.', `${s1}`],
              ['2', 'Exterior of UCR', 'Inspect the UCR exterior for noticeable scratch marks ,grime and/or deformation that may affect the functionality. Inspect the tube connected areas on the front panel for adherence of patient body fluid( e.g. blood)', `${s2}`],
              ['3', 'The status of gas supply from CO2 gas outlet just after turning ON the equipment', 'The gas flow LED lights out just after turning ON the equipment', `${s3}`],
              ['4', 'Gas supply function', 'When the Start/ Stop switch is pressed, the electromagnetic valve will be open and the equipment starts supplying CO2 gas.\nWhen the Start/ Stop switch is pressed, the electromagnetic valve will be closed and the equipment stops supplying CO2 gas.', `${s4}`],
              ['5', 'Turn off the power', ' Press the power switch of the UCR.\nConfirm that the power of the UCR is turned off, and the power switch indicator goes out', `${s5}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'UCR_' + sno}`; // Custom file name
