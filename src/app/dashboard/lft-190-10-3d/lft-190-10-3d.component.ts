...
@Component({
  selector: 'app-lft-190-10-3d',
  templateUrl: './lft-190-10-3d.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Lft190103dComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR LTF-190-10-3D', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['3', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation\n2. Inspection of Angulation lock in free position.\n3. Inspection of the UP/DOWN and RIGHT/LEFT angulation mechanism', `${s3}`],
              ['4', 'Endoscope connector', 'Connection of ancillary equipment:\n1. Connection to the light source\n2. Connection to the video connector socket (MASTER/ SLAVE)', `${s4}`],
              ['5', 'Image system', 'Inspection of the endoscopic image (Use 3D glasses supplied with THE 3D monitor and processor)', `${s5}`],
              ['6', 'Control section;\nswitch system', 'Inspection of the remote switches', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'LTF-190-10-3D_' + sno}`; // Custom file name
