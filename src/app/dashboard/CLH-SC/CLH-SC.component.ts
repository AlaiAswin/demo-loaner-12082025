...
@Component({
  selector: 'app-CLH-SC',
  templateUrl: './CLH-SC.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Clhsccomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR CLH-SC', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the CLH-SC must be performed according to the Instructions Manual.', `${s1}`],
              ['2', 'Power ON', 'Confirm that the lamp cover on the left side of the CLH-SC is closed completely.\nPress the power switch of CLH-SC to turn on the power.\nConfirm the following:\nThe green power switch indicator is illuminated.\nThe cooling fan motor starts operating.', `${s2}`],
              ['3', 'Main Lamp', 'Connect the endoscope to CLH-SC.\nConfirm the following:\nPress the power switch of the CLH-SC to turn on the power.\nConfirm that light is emitted from the distal end of the endoscope.\nPress the power switch of the CLH-SC to turn off the power.\nConfirm that light from the distal end of the endoscope goes out.', `${s3}`],
              ['4', 'Brightness control', 'Press the power switch of the CLH-SC to turn on the power.\nVerify that the brightness can be adjusted properly.\nConfirm the following:\nConfirm that the brightness increases as you turn the brightness control knob clockwise.\nConfirm that the brightness decreases as you turn the brightness control knob counter-clockwise.', `${s4}`],
              ['5', 'Turn off the power', ' Press the power switch of the CLH-SC.\nConfirm that the power of the CLH-SC is turned off, and the power switch indicator goes out', `${s5}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'CLH-SC_' + sno}`; // Custom file name
