...
@Component({
  selector: 'app-imh-200',
  templateUrl: './imh-200.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Imh200Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR IMH-200', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'The IMH-200 must be cleaned and disinfected according to instructions described in the Instructions Manual.', `${s1}`],
              ['2', 'Exterior of IMH-200', 'Inspect the exterior of the IMH-200 for noticeable scratches, grime, and/or deformation that may affect the functionality..', `${s2}`],
              ['3', 'Power ON', 'Press the Power switch on the rear panel. Inspect the following item: The Power switch on the front panel illuminates in green. \nPress the Power switch on the front panel. Inspect the following items:\n・ IMH-10: Messages “Welcome,” “Booting….,” and “NO ID” appear on the front panel display successively.\n・ IMH-200: The OLYMUS logo appears on the front touch panel display', `${s3}`],
              ['4', 'Endoscope image display', '1. After the endoscope image is input to the IMH-200 from the video processor, display the image on the monitor. Inspect the following items:\n・ IMH-10: Press the MENU button on the front panel for one second or more.\nThe MENU screen appears.\n・ IMH-200: The endoscope image appears in the PORT A area of the front touch panel display.\n2. Press the CAPTURE button on the infrared remote controller. Inspect the following items:\n・ IMH-10: The counter “CAPTURE-***” appears on the front touch panel display.\n・ IMH-200: A numeric of the capture counter on the front touch panel display increases.\n3. Press the REC button on the infrared remote controller to record the image.\nPressthe STOP button to stop recording. Inspect the following item:\n・ Press the PLAY button. The recorded image is played back.', `${s4}`],
              ['5', 'Infrared remote controller', 'IMH-10: Press the MENU button on the infrared remote controller. Verify that the MENU screen appears on the monitor. IMH-200: Press the AUDIO button on the remote controller. Verify that AUDIO display switches between ON and OFF.', `${s5}`],
              ['6', 'Image data recording', 'Insert a BD or DVD into the BD/DVD drive. Set the following items in the Storage media tab:\n Save destination: BD/DVD Additional media: SYSTEM1 Copy destination: SYSTEM1 Playback: Yes\n Press the REC or CAPTURE button. Verify that the BD/DVD indicator on the front panel blinks in green.', `${s6}`],
              ['7', 'USB storage media', 'Connect a USB memory stick to the SYSTEM1 or SYTEM2 terminal. Verify that the SYTEM1 or SYTEM2 indicator on the front panel does not illuminate in orange.', `${s7}`],
              ['8', 'Data Delete and Reset', 'Delete and Reset previous data, ie user setting, saved image, patient data etc.', `${s8}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'IMH-200_' + sno}`; // Custom file name
