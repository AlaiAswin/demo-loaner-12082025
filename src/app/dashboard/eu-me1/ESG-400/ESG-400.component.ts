...
@Component({
  selector: 'app-ESG-400',
  templateUrl: './ESG-400.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Esg400component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - ESG-400', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
...

        {
          table: {
            widths: [25, 125, 275, 75],
            body: [
              ['4', 'SELF TEST ESG-400 ', 'PROCEDURE PASSED', `${s4}`],
              ['5', 'DISPLAY & SOUND', 'PROCEDURE PASSED', `${s5}`],
              ['6', 'PUSH BUTTONS', ' PROCEDURE PASSED', `${s6}`],
              ['7', 'FOOT SWITCH CONNECTION', ' PROCEDURE PASSED', `${s7}`],
              ['8', 'COMMUNICATION TEST (Connection LINK-IN - LINK-OUT)', ' PROCEDURE PASSED', `${s8}`],
              ['9', 'FACTORY DEFAULT (Optional)', ' PROCEDURE PASSED', `${s9}`],
            ],
          }
        },
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'ESG-400_' + sno}`; // Custom file name
