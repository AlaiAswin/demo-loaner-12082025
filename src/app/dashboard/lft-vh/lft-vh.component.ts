...
@Component({
  selector: 'app-lft-vh',
  templateUrl: './lft-vh.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class LftvhComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR LTF-VH', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'LTF-VH_' + sno}`; // Custom file name
