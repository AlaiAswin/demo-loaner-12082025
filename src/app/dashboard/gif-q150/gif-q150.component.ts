...
@Component({
  selector: 'app-gif-q150',
  templateUrl: './gif-q150.component.html',
  styleUrls: ['./gif-q150.component.css'],
  imports: [CommonModule, ReactiveFormsModule, FormsModule],
  standalone: true,
})
export class GifQ150Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    const sn = document.getElementById('sn_d') as HTMLInputElement;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14a == p) && (s14b == p) && (s15 == p) && (s16 == p)) {
          overRes = 'Pass'
          console.log('Pass')
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-Q150', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-Q150_' + sno}`; // Custom file name
