...
@Component({
  selector: 'app-gif-hq190',
  templateUrl: './gif-hq190.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class GifHq190Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder,
    ...
    this.formGroup = this.fb.group({
      ...
      s11a_result: [, Validators.required],
      s11b_result: [, Validators.required],
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        let s11a = data['s11a_result'];
        let s11b = data['s11b_result'];
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11a == p) && (s11b == p) && (s12 == p) && (s13 == p) && (s14a == p) && (s14b == p) && (s15 == p) && (s16 == p) && (s17 == p)) {
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-HQ190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['11.1', 'Image system', 'Inspection of the focus switching function:\n1. Inspection of switching to the near focus mode', `${s11a}`],
                ['11.2', 'Image system', 'Inspection of the focus switching function:\n2. Inspection of switching to the normal focus mode', `${s11b}`],
                ['12', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s12}`],
                ['13', 'Water supply system', 'Inspection of the air-feeding function', `${s13}`],
                ['14.1', 'Air supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function', `${s14a}`],
                ['14.2', 'Air supply system', 'Inspection of the objective lens cleaning function:\n2. Inspection of removing the remaining water from the objective lens', `${s14b}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-HQ190_' + sno}`; // Custom file name
