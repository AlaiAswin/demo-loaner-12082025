...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-TJF-145',
  templateUrl: './TJF-145.component.html',
  styleUrls: ['./TJF-145.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Tjf145Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        { text: 'DEMO-LOANER INSPECTION SHEET FOR TJF-145', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
      ...
      if (image_data_url) {
        content.unshift(
          { text: 'Issue date : 27 Sep 2023 ', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 00', alignment: 'right', fontSize: 5 },
          {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'TJF-145_' + sno}`; // Custom file name
