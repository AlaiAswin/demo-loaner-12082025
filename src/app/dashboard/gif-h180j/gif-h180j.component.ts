...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-gif-h180j',
  templateUrl: './gif-h180j.component.html',
  styleUrls: ['./gif-h180j.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Gifh180jComponent {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-H180J', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-H180J_' + sno}`; // Custom file name
