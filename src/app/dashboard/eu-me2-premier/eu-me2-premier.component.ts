...
@Component({
  selector: 'app-eu-me2-premier',
  templateUrl: './eu-me2-premier.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Eume2premierComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR EU-ME2_PREMIER', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning/disinfection', 'Clean and disinfect the EU-ME2 PREMIER according to the instructions described in the corresponding Instructions Manual.', `${s1}`],
              ['2', 'Power ON', 'Verify that the air vents on the left and right sides of the EU-ME2 PREMIER are not clogged with dusts and/or foreign substances.\n- Connect an ultrasound endoscope (*) to the EU-ME2 PREMIER.\n- Press the power switch on the EU-ME2 PREMIER.\nInspect the following items:\n・ The power indicator lights up.\n・ One minute after power-on, a beep sounds twice.\n- Place a hand in front of the air vents on the left side of the EU-ME2 PREMIER.\n- Verify by feeling that the air is exhausted from the air vent on the left side', `${s2}`],
              ['3', 'Display contents', 'With the EU-ME2 PREMIER powered ON,inspect the following items of the monitor display:\nThe date and time displayed on the monitor are correct.', `${s3}`],
              ...
              ['11', 'Video system center ', 'Connect the video system center to EU-ME2 PREMIER.\n- Turn the video system center and EU-ME2 PREMIER on.\n- Verify the correct model name of the video system center is indicated on upper left of the keyboard touch panel.', `${s11}`],
              ['12', 'Power OFF ', 'Press the power switch on the EU-ME2 PRMIER PLUS.\n- Verify that the EU-ME2 PREMIER PLUS is turned off and the power indicator for the Power switch extinguishes.', `${s12}`], ['13', 'Data Delete and Reset', 'Delete and Reset previous data, ie user setting, saved image, patient data etc.', `${s13}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'EU-ME2 PREMIER_' + sno}`; // Custom file name
