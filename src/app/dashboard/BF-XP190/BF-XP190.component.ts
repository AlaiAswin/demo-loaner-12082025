...
@Component({
  selector: 'app-BF-XP190',
  templateUrl: './BF-XP190.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class BfxP190Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-XP190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Insertion section', 'Inspection of the insertion tube rotation mechanism', `${s4}`],
                ['5', 'Endoscope connector', 'Connection to the light source', `${s5}`],
                ['6', 'Image system', 'Inspection of the endoscopic image', `${s6}`],
                ['7', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s7}`],
                ['8', 'Water supply system', 'Inspection of the water feeding function', `${s8}`],
                ['9', 'Suction System', 'Inspection of the suction function', `${s9}`],
                ['10', 'Instrument channel system', 'Inspection of the instrument channel', `${s10}`],
        ...
        if (image_data_url) {
          content.unshift(
            { text: 'Issue date : 25 Aug 2023', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 00', alignment: 'right', fontSize: 5 },
            {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-XP190_' + sno}`; // Custom file name
