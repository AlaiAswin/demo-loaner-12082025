...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-PCF-Q180AL',
  templateUrl: './PCF-Q180AL.component.html',
  styleUrls: ['./PCF-Q180AL.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Pcfq180alComponent {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p) && (s17 == p) && (s18 == p) && (s19 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR PCF-Q180AL', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['4', 'Insertion section', 'Inspection of the flexibility adjustment mechanism', `${s5}`],
                ['5', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation\n2. Inspection of the UP/DOWN/RIGHT/LEFT angulation mechanism', `${s6}`],
                ...
                ['9', 'Endoscope connector', 'Connection to the light source', `${s10}`],
                ['10', 'Endoscope connector', 'Connection of the water container', `${s11}`],
                ['11', 'Endoscope connector', 'Connection to the videoscope cable', `${s13}`],
                ['12', 'Image system', 'Inspection of the endoscopic image', `${s12}`],

                ['13', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s14}`],
                ['14', 'Air supply system', 'Inspection of the air-feeding function:1. Confirmation of emitting no air bubbles\n2. Confirmation of emitting air bubbles', `${s15}`],
                ...(image_data_url !== null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
                ['15', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s16}`],
                ['16', 'Suction System', 'Inspection of the suction function', `${s17}`],
                ['17', 'Auxiliary water system', 'Inspection of the auxiliary water feeding function', `${s18}`],
                ['18', 'Instrument channel system', 'Inspection of the instrument channel', `${s19}`],
                ...(image_data_url === null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'PCF-Q180AL_' + sno}`; // Custom file name
