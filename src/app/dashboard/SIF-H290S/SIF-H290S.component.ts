...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-SIF-H290S',
  templateUrl: './SIF-H290S.component.html',
  styleUrls: ['./SIF-H290S.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Sifh290sComponent {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR SIF-H290S', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'SIF-H290S_' + sno}`; // Custom file name
