...
@Component({
  selector: 'app-PSF-1',
  templateUrl: './PSF-1.component.html',
  styleUrls: ['./PSF-1.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Psf1Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder,
    ...
    , private router: Router, private renderer: Renderer2, private location: Location) {
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p) && (s17 == p) && (s18 == p) && (s19 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR PSF-1', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['8', 'Endoscope connector', 'Connection to the light source', `${s10}`],
                ['9', 'Endoscope connector', 'Connection of the water container', `${s11}`],
                ['10', 'Image system', 'Inspection of the endoscopic image', `${s12}`],

                ['11', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s14}`],
                ['12', 'Air supply system', 'Inspection of the air-feeding function', `${s15}`],
                ...(image_data_url !== null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
                ['13', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s16}`],
                ['14', 'Suction System', 'Inspection of the suction function', `${s17}`],

                ['15', 'Instrument channel system', 'Inspection of the instrument channel', `${s19}`],
                ['16', 'Endoscopic system', 'Inspection of maximum rotation count at No-Load', `${s4}`],
                ['17', 'Endoscopic system', 'Inspection of R-rubber replacement indication functions', `${s5}`],
                ['18', 'Endoscopic system', 'Inspection of PSID functions', `${s13}`],
                ['19', 'Endoscopic system', 'Inspection of motor drive limit during high load (Manual Check)', `${s18}`],
        ...
        if (image_data_url) {
          content.unshift(
            { text: 'Issue date : 05 Sep 2022', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 01', alignment: 'right', fontSize: 5 },
            {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'PSF-1_' + sno}`; // Custom file name
