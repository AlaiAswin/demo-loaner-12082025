...
@Component({
  selector: 'app-bf-3c40',
  templateUrl: './bf-3c40.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Bf3C40Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-3C40', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['5', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s5}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-3C40_' + sno}`; // Custom file name
