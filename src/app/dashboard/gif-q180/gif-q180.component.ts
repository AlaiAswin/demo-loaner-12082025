...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-gif-q180',
  templateUrl: './gif-q180.component.html',
  styleUrls: ['./gif-q180.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Gifq180Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-Q180', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-Q180_' + sno}`; // Custom file name
