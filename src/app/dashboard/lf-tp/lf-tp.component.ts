...
@Component({
  selector: 'app-lf-tp',
  templateUrl: './lf-tp.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class LftpComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR LF-TP', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'LF-TP_' + sno}`; // Custom file name
