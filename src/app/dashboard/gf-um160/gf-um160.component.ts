...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-gf-um160',
  templateUrl: './gf-um160.component.html',
  styleUrls: ['./gf-um160.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class GfUM160Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GF-UM160', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['8', 'Endoscope connector', 'Connection of the water container', `${s8}`],
                ['9', 'Endoscope connector', 'Connection of the videoscope cable', `${s9}`],
                ['10', 'Endoscope connector', 'Connection of the endoscope and ultrasonic cable', `${s10}`],
                ['11', 'Image system', 'Inspection of the endoscopic image', `${s11}`],
                ['12', 'Ultrasound imaging system', 'Inspection of the ultrasound image with the Olympus universal endoscopic ultrasound center EU-ME1', `${s12}`],
                ['13', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s13}`],
                ['14', 'Air supply system', 'Inspection of the air-feeding function', `${s14}`],
                ['15', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s15}`],
                ['16', 'Balloon water supply system', 'Inspection of the water feeding function into the balloon', `${s16}`],
                ['17', 'Suction System', 'Inspection of the suction function', `${s17}`],
                ['18', 'Balloon water suction system', 'Inspection of aspiration from the balloon water suction port', `${s18}`],
                ['19', 'Instrument channel system;\nforceps elevator system', 'Inspection of the instrument channel and forceps elevator', `${s19}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GF-UM160_' + sno}`; // Custom file name
