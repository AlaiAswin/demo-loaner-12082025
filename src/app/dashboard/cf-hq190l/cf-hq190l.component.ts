...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-cf-hq190l',
  templateUrl: './cf-hq190l.component.html',
  styleUrls: ['./cf-hq190l.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class CfHq190lComponent {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder, private popupservice: PopUpService, private router: Router, private renderer: Renderer2, private location: Location) {
    this.formGroup = this.fb.group({
      ...
      s6a_result: [, Validators.required],
      s6b_result: [, Validators.required],
      ...
      s13a_result: [, Validators.required],
      s13b_result: [, Validators.required],
      s14_result: [, Validators.required],
      s15_result: [, Validators.required],
      s16_result: [, Validators.required],
      s17a_result: [, Validators.required],
      s17b_result: [, Validators.required],
      s18_result: [, Validators.required],
      s19_result: [, Validators.required],
      s20_result: [, Validators.required],
      s21_result: [, Validators.required],
      s22_result: [, Validators.required],
      remarks: []
    });
  }
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        let s6a = data['s6a_result'];
        let s6b = data['s6b_result'];
        ...
        let s13a = data['s13a_result'];
        let s13b = data['s13b_result'];
        let s14 = data['s14_result'];
        let s15 = data['s15_result'];
        let s16 = data['s16_result'];
        let s17a = data['s17a_result'];
        let s17b = data['s17b_result'];
        let s18 = data['s18_result'];
        let s19 = data['s19_result'];
        let s20 = data['s20_result'];
        let s21 = data['s21_result'];
        let s22 = data['s22_result'];
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6a == p) && (s6b == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13a == p) && (s13b == p) && (s14 == p) && (s15 == p) && (s16 == p) && (s17a == p) && (s17b == p) && (s18 == p) && (s19 == p) && (s20 == p) && (s21 == p) && (s22 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR CF-HQ190L', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['4', 'Insertion section', 'Inspection of the passive bending section', `${s4}`],
                ['5', 'Insertion section', 'Inspection of the flexibility adjustment mechanism', `${s5}`],
                ['6.1', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation', `${s6a}`],
                ['6.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the UP/DOWN/RIGHT/LEFT angulation mechanism', `${s6b}`],
                ['7', 'Control section;\nSuction valve mounting section', 'Attaching the suction valve', `${s7}`],
                ['8', 'Control section;\nAir/water valve mounting section', 'Attaching the air/water valve', `${s8}`],
                ['9', 'Control section;\nBiopsy valve mounting section', 'Attaching the biopsy valve', `${s9}`],
                ['10', 'Endoscope connector', 'Connection to the light source', `${s10}`],
                ['11', 'Endoscope connector', 'Connection of the water container', `${s11}`],
                ['12', 'Image system', 'Inspection of the endoscopic image', `${s12}`],
                ['13.1', 'Image system', 'Inspection of the focus switching function:\n1. Inspection of switching to the near focus mode', `${s13a}`],
                ['13.2', 'Image system', 'Inspection of the focus switching function:\n2. Inspection of switching to the normal focus mode', `${s13b}`],
                ['14', 'Insertion section', 'Endoscope position detecting function (UPD function check)', `${s14}`],
                ['15', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s15}`],
                ['16', 'Air supply system', 'Inspection of the air-feeding function', `${s16}`],
                ['17.1', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function', `${s17a}`],
                ['17.2', 'Water supply system', 'Inspection of the objective lens cleaning function:\n2. Inspection of removing the remaining water from the objective lens', `${s17b}`],
                ['18', 'Suction System', 'Inspection of the suction function', `${s18}`],
                ['19', 'Biopsy Channel', 'Inspection of the instrument channel', `${s19}`],
                ['20', 'Auxiliary water system', 'Inspection of the auxiliary water feeding function', `${s20}`],
                ['21', 'Instrument channel system', 'Inspection of the instrument channel', `${s21}`],
                ['22', 'Insertion section', 'Inspection of insertion tube insulation', `${s22}`],
              ],
            }
          },
          {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'CF-HQ190L_' + sno}`; // Custom file name
