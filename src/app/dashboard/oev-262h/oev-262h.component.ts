...
@Component({
  selector: 'app-oev-262h',
  templateUrl: './oev-262h.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Oev262hComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OEV-262H', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Clean and disinfect the OEV262H according to the instructions described in the corresponding Instructions Manual.', `${s1}`],
              ['2', 'Power ON', '・ Press the power switch on the OEV262H.\n・Verify that the power indicator on the monitor lights up green.', `${s2}`],
              ['3', 'Button operation', 'Press all buttons on the front panel of the OEV262H', `${s3}`],
              ['4', 'Monitor screen', 'Connect the video system center to the OEV262H with video cables (SDI, DVI, Svideo, Video) to transfer the color bar image.\n・Connect the PC to the OEV262H with the HD15 cable to transfer PC images.\n・Turn ON the connected equipment.\n・Select each of the SDI, DVI, S-video, and Video signals for image display on the monitor.\nInspect the following items:\n・ The image is displayed on the monitor according to the selected signal.\n・ There are no discoloration, noise, and distortion of the image displayed on the monitor.\n・ Repeat the above steps to inspect all the signals.\n・ Inspect the LCD panel on the monitor for cracks.', `${s4}`],
              ['5', 'Output signal', 'Turn OFF the OEV262H.\n・Connect the output terminal and secondary monitor with video cables. The secondary monitor is equipped with terminals such as SDI, DVI, S-Video, Video,HD15.\n・ Turn ON both monitors.\n・ Set the same signal (SDI, DVI, S-Video, Video or HD15) for image display on both monitors.\nInspect the following items:\n・ The image on the OEV262H is automatically transferred to the secondary monitor.\n・ There are no discoloration, noise, and distortion of the image displayed on the secondary monitor.\n・Repeat the above steps to inspect all the signals that the error window is displayed with E394: Communication error.', `${s5}`],
              ['6', 'Power OFF', 'Press the power switch on the OEV262H.', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OEV-262H_' + sno}`; // Custom file name
