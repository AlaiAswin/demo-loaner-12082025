...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-GIF-1TH190',
  templateUrl: './GIF-1TH190.component.html',
  styleUrls: ['./GIF-1TH190.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Gif1th190Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-1TH190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-1TH190_' + sno}`; // Custom file name
