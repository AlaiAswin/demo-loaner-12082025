...
@Component({
  selector: 'app-cyf-5a',
  templateUrl: './cyf-5a.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Cyf5aComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR CYF-5A', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Control section;\nSuction valve mounting section', 'Attaching the suction valve', `${s4}`],
                ['5', 'Control section;\light guide connector', 'Inspection of the endoscopic image by rechargeable miniature light source', `${s5}`],
                [
                  '6',
                  'Image system',
                  {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'CYF-5A_' + sno}`; // Custom file name
