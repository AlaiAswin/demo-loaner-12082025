...
@Component({
  selector: 'app-maf-tm',
  templateUrl: './maf-tm.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class MaftmComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR MAF-TM', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['2', 'Memory card/battery', 'Inserting the memory card/loading the battery', `${s2}`],
                ['3', 'Camera section', 'Inspection of the camera section\n1. Inspection of the endoscopic image\n2. Inspection of the remote switches', `${s3}`],
                ['4', 'Overall appearance', 'Inspection of the endoscope', `${s4}`],
                ['5', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation\n2. Inspection of the UP/DOWN angulation mechanism', `${s5}`],
                ['6', 'Suction System', 'Inspection of the suction function', `${s6}`],
                ['7', 'Water supply system', 'Inspection of the water feeding function', `${s7}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'MAF-TM_' + sno}`; // Custom file name
