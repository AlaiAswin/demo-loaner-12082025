...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-gif-2th180',
  templateUrl: './gif-2th180.component.html',
  styleUrls: ['./gif-2th180.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Gif2th180Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-2TH180', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['5', 'Attaching accessories', '1. Attaching the suction valve\n2. Attaching the air/water valve\n3. Attaching the biopsy valve\n4. Attaching the auxiliary water inlet cap', `${s5}`],
                ['6', 'Endoscope connector', 'Connection of the water container', `${s6}`],
                ['7', 'Endoscope connector', 'Connection of the videoscope cable', `${s7}`],
                ['8', 'Endoscope connector', 'Connection of the endoscope and ultrasonic cable', `${s8}`],
                ['9', 'Image system', 'Inspection of the endoscopic image', `${s9}`],
                ['10', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s10}`],
                ['11', 'Air supply system', 'Inspection of the air-feeding function', `${s11}`],
                ['12', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s12}`],
                ['13', 'Suction System', 'Inspection of the suction function', `${s13}`],
                ['14', 'Instrument channel system', 'Inspection of the instrument channel', `${s14}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GIF-2TH180_' + sno}`; // Custom file name
