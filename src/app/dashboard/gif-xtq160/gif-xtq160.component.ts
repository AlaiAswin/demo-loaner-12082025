...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-gif-xtq160',
  templateUrl: './gif-xtq160.component.html',
  styleUrls: ['./gif-xtq160.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class GifXTQ160Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GIF-XTQ160', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'XTQ160_' + sno}`; // Custom file name
