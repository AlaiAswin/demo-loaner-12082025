...
@Component({
  selector: 'app-demo-dv-loaner',
  templateUrl: './demo-dv-loaner.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class DemoDvLoanerComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR 3DV-190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the 3DV-190 must be performed according to the Instructions Manual.', `${s1}`],
              ['2', 'Power ON', 'Confirm that the ventilation grills on both side panels of 3DV-190 are not covered with dust or other materials.\n Connect each tester and jig to 3DV-190 using suitable cables.\n Connect the endoscope to the video system center and the xenon light source.\n Press the power switch of the 3DV-190.\nConfirm that the power indicator is illuminated.\n Confirm that air is exhausted by holding your hand in front of the ventilation grills on the left side panel.', `${s2}`],
              ['3', 'Monitor Display', 'Turn on the power of testers and jigs.\n Confirm that the monitor displays the date, time, and 2D/3D status.\n Confirm that the correct date and time are displayed.\n Hold the endoscope’s distal end towards your palm.\n Confirm that the monitor displays the image of palm normally, and no noise is heard.', `${s3}`],
              ['4', '2D/3D Switch Function', 'Press DISPLAY MODE button for several times, and confirm that the display mode is changed between 3D and 2D modes.\n Confirm the following:\n Press DISPLAY MODE button to switch to 3D display.\n Confirm that the letter of “3D” displayed on the upper right corner of the display looks horizontally doubled, not vertically.\n Confirm that the display mode corresponds with the\n “DISPLAY MODE” indicator on the front panel.', `${s4}`],
              ...(image_data_url !== null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
              ['5', 'Turn off the power', 'Press the power switch of peripheral devices to turn off.\n Press the power switch of the 3DV-190.\n Confirm that the power of the 3DV-190 is turned off, and the power switch indicator goes out.', `${s5}`],
        ...
        function () {
          if (image_data_url == null) {
            // Return an empty text element without a page break
            return { text: '', pageBreak: 'after' };
          } else {
            // Return an empty text element with a page break
            return { text: '' };
          }
        }(),
        {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'3DV-190_' + sno}`; // Custom file name
