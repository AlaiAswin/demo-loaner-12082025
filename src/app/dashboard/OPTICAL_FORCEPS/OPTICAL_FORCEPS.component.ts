...
@Component({
  selector: 'app-OPTICAL_FORCEPS',
  templateUrl: './OPTICAL_FORCEPS.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Opticalforcepscomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - OPTICAL_FORCEPS', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
...

        {
          table: {
            ...
            body: [
              ...
              ['5', 'HANDLE OPERATION', 'PROCEDURE PASSED', `${s5}`],
              ['6', 'JAW INSERTS', ' PROCEDURE PASSED ', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OPTICAL_FORCEPS_' + sno}`; // Custom file name
