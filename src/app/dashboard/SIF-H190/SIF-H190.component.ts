...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-SIF-H190',
  templateUrl: './SIF-H190.component.html',
  styleUrls: ['./SIF-H190.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Sifh190Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR SIF-H190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['10', 'Image system', 'Inspection of the endoscopic image', `${s11}`],
                ['11', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s12}`],
                ['12', 'Air supply system', 'Inspection of the air-feeding function', `${s13}`],
                ...
                ['14', 'Suction System', 'Inspection of the suction function', `${s15}`],
                ['15', 'Instrument channel system', 'Inspection of the instrument channel', `${s16}`],
        ...
        if (image_data_url) {
          content.unshift(
            { text: 'Issue date : 08 Aug 2023', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 00', alignment: 'right', fontSize: 5 },
            {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'SIF-H190_' + sno}`; // Custom file name
