...
@Component({
  selector: 'app-bf-1tq170',
  templateUrl: './bf-1tq170.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Bf1tq170Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5a == p) && (s5b == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-1TQ170', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['2', 'Forceps Passage', 'Perform the forceps by angulating in Up direction.', `${s2}`],
                ['3', 'Brush Passage', 'Channel cleaning brush passage', `${s3}`],
                ['4', 'Overall appearance', 'Inspection of the endoscope', `${s4}`],
                ['5.1', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation', `${s5a}`],
                ['5.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the UP/DOWN angulation mechanism', `${s5b}`],
                ['6', 'Control section;\nSwitch section', 'Control section;\nSuction valve mounting section', `${s6}`],
                ...
                ['11', 'Instrument channel system', 'Inspection of the instrument channel', `${s11}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-1TQ170_' + sno}`; // Custom file name
