...
@Component({
  selector: 'app-enf-vq',
  templateUrl: './enf-vq.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class EnfVqComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR ENF-VQ', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'ENF-VQ_' + sno}`; // Custom file name
