...
@Component({
  selector: 'app-cf-ez1500dl',
  templateUrl: './cf-ez1500dl.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class CFEZ1500DLComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder, private popupservice: PopUpService, private router: Router, private renderer: Renderer2, private location: Location) {
    this.formGroup = this.fb.group({
      ...
      s8a_result: [, Validators.required],
      s8b_result: [, Validators.required],
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        let s8a = data['s8a_result'];
        let s8b = data['s8b_result'];
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6a == p) && (s6b == p) && (s7 == p) && (s8a == p) && (s8b == p) && (s9 == p) && (s10 == p)
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR CF-EZ1500DL', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['6.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the UP/DOWN angulation mechanism', `${s6b}`],
                ['7', 'Image system', 'Inspection of the endoscopic image', `${s7}`],
                ['8.1', 'Image system', 'Inspection of the focus switching function:\n1. Inspection of switching to the near focus mode', `${s8a}`],
                ['8.2', 'Image system', 'Inspection of the focus switching function:\n2. Inspection of switching to the normal focus mode', `${s8b}`],
                ['9', 'Insertion section', 'Endoscope position detecting function (UPD function check)', `${s9}`],
                ['10', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s10}`],
                ['11', 'Air supply system', 'Inspection of the air-feeding function', `${s11}`],
                ['12', 'Objective lens', 'Inspection of the objective lens cleaning function', `${s12}`],
                ['13', 'Suction System', 'Inspection of the suction function', `${s13}`],
                ['14', 'Biopsy Channel', 'Inspection of the instrument channel', `${s14}`],
                ['15', 'Auxiliary water system', 'Inspection of the auxiliary water feeding function', `${s15}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'CF-EZ1500DL_' + sno}`; // Custom file name
