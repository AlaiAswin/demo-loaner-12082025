...
@Component({
  selector: 'app-OTV-SC2',
  templateUrl: './OTV-SC2.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Otvsc2component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OTV-SC2', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Exterior confirmation', 'No significant scratch, dust and distortion', `${s1}`],
              ['2', 'Power on confirmation', 'The power on indicator must be lit when the unit is powered on.', `${s2}`],
              ['3', 'White balance function confirmation', 'The white balance function must work properly.\nCheck with camera head  CH-SC2-1D.', `${s3}`],
              ['4', 'Brightness adjustment function check', 'The brightness must be change accordingly when adjustment button is depressed on the rear panel.\nCheck with camera head  CH-SC2-1D.', `${s4}`],
              ['5', 'Exposure area switching function confirmation', 'The exposure area switching function must switch the exposure area.', `${s5}`],
              ['6', 'Signal output', 'Video and Y/C are working okay.', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OTV-SC2_' + sno}`; // Custom file name
