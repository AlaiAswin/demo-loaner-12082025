...
@Component({
  selector: 'app-ESG-150',
  templateUrl: './ESG-150.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Esg150component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - ESG-150/ESG-200/CELON PRECISION', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
...

        {
          table: {
            widths: [25, 125, 275, 75],
            body: [
              ['4', 'SELF TEST', 'PROCEDURE PASSED', `${s4}`],
              ['5', 'DISPLAY & SOUND', 'PROCEDURE PASSED', `${s5}`],
              ['6', 'FOOT SWITCH CONNECTION', ' PROCEDURE PASSED', `${s6}`],
              ['7', 'FACTORY DEFAULT(Optional)', ' PROCEDURE PASSED', `${s7}`],
            ],
          }
        },
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'ESG-150-200-CELON_' + sno}`; // Custom file name
