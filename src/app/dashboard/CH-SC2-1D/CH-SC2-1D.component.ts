...
@Component({
  selector: 'app-CH-SC2-1D',
  templateUrl: './CH-SC2-1D.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Chsc21dComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR CH-SC2-1D', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'The CH-SC2-1D must be cleaned and disinfected according to instructions described in the Instructions Manual.', `${s1}`],
              ['2', 'Inspection of the camera head', 'Confirm that the camera head does not have the following damages.\nSurface of the camera head’s main body is free from cracks, burrs, or other damages.\nCamera cable is free from sagging, swelling, scratches, or holes.\nElectrical contacts of the video connector are free from bends or rust.\nMake sure that the equipment is completely dry before use. Wipe the video connector,including the electrical contacts using clean lint-free cloths. Also confirm that the electrical contacts are completely dry and clean.', `${s2}`],
              ['3', 'Connection to the camera control unit', 'If any ancillary equipment is ON, turn it OFF.\nConfirm that the video connector, including electrical contacts and optical connecting part, is dry and clean.\nMake sure that the UP mark on the video connector is facing up.\nHold the camera control unit stationary with one hand. With the other hand, push the video connector into the video connector socket until it clicks.\nConfirm that the video connector is securely attached by pulling it gently.', `${s3}`],
              ...
              ['5', 'Inspection of the remote switches', 'Turn ON the camera control unit, the light source,and the monitor.\nObserve the palm of your hand in the WLI and NBI endoscopic images.\nConfirm that the illumination light is emitted and that the WLI and NBI endoscopic images are free from noise, blur, fog, or other irregularities.\nShake the camera cable and confirm that the endoscopic image does not momentarily disappear, flicker, or display any other irregularities', `${s5}`],
              ['6', 'Function check', 'Each function works properly.', `${s6}`],
              ['7', 'Image quality check', 'The image quality of the endoscopic image on the LCD monitor must be as follows:\nThe endoscopic image on the monitor has very little noise.\nColors are rendered correctly and evenly. -The brightness of the image is correct and free of unevenness. -There are no smears, missing pixels, or ghost images which may cause significant troubles when observing endoscopic images. -There is no dust on the imaging device.', `${s7}`],
              ['8', 'Endoscopic image check', 'When the camera head moves up or down, the endoscopic image moves up or down in the same direction.', `${s8}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'CH-SC2-1D_' + sno}`; // Custom file name
