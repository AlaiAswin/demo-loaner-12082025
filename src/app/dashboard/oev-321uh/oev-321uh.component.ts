...
@Component({
  selector: 'app-oev-321uh',
  templateUrl: './oev-321uh.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Oev321uhComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OEV-321UH', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Clean and disinfect the OEV321UH according to the instructions described in the corresponding Instructions Manual.', `${s1}`],
              ['2', 'Power ON', 'Press the power switch on the OEV321UH.\nVerify that the power indicator on the monitor lights up green', `${s2}`],
              ['3', 'Button operation', 'Press all buttons on the front panel of the OEV321UH.', `${s3}`],
              ['4', 'Monitor screen', 'Connect the video system center ( CV-1500) to the OEV321UH with video cables SDI to transfer the color bar image.・\nTurn ON the connected equipment.・\nSelect each of the 12G SDI, & 3G SDI Video signals for image display on the monitor.\nInspect the following items:・\nThe image is displayed on the monitor according to the selected signal.・\nThere are no discoloration, noise, and distortion of the image displayed on the monitor.・\nRepeat the above steps to inspect all the signals.・\nInspect the LCD panel on the monitor for cracks.', `${s4}`],
              ['5', 'Output signal', 'There shall be no damaged, deformed or dirty appearnace on exterior.\nLables and marking attached on the main unit shall be completely legible.\nThere shall be no damaged , deformed or dirty appearance on exterior of the cable cover.', `${s5}`],
              ['6', 'Power OFF', 'Press the power switch on the OEV321UH.', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OEV-321UH_' + sno}`; // Custom file name
