...
@Component({
  selector: 'app-ESG-300',
  templateUrl: './ESG-300.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Esg300component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - ESG-300', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        { text: 'VISUAL INSPECTION OF THE ELECTRO SURGICAL GENERATOR AND ACCESSORIES', fontSize: 12 },
        {
          table: {
            ...
            body: [
              ...
              ['3', 'OBSTRUCTION,OR DAMAGE', 'NON PRESENT', `${s3}`],
...

        {
          table: {
            widths: [25, 125, 275, 75],
            body: [
              ['4', 'SELF TEST ESG-300 ', 'PROCEDURE PASSED', `${s4}`],
              ['5', 'POWER UP APU-300', 'PROCEDURE PASSED', `${s5}`],
              ['6', 'DISPLAY & SOUND (ESG-300)', ' PROCEDURE PASSED', `${s6}`],
              ['7', 'FOOT SWITCH CONNECTION (ESG-300 ONLY)', ' PROCEDURE PASSED', `${s7}`],
              ['8', 'COMMUNICATION', ' PROCEDURE PASSED', `${s8}`],
              ['9', 'FACTORY DEFAULT (Optional ESG-300 Only)', ' PROCEDURE PASSED', `${s9}`],
            ],
          }
        },
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'ESG-300_' + sno}`; // Custom file name
