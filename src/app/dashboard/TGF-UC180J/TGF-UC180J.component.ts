...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-TGF-UC180J',
  templateUrl: './TGF-UC180J.component.html',
  styleUrls: ['./TGF-UC180J.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Tgfuc180jComponent {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR TGF-UC180J', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['8', 'Endoscope connector', 'Connection to the light source', `${s19}`],
                ['9', 'Endoscope connector', 'Connection of the water container', `${s8}`],
                ['10', 'Endoscope connector', 'Connection of the videoscope cable', `${s9}`],
                ['11', 'Endoscope connector', 'Connection of the endoscope and ultrasonic cable', `${s10}`],
                ['12', 'Image system', 'Inspection of the endoscopic image', `${s11}`],
                ['13', 'Ultrasound imaging system', 'Inspection of the ultrasound image with the Olympus universal endoscopic ultrasound center EU-ME1', `${s12}`],
                ['14', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s13}`],
                ['15', 'Air supply system', 'Inspection of the air-feeding function', `${s14}`],
                ...
                ['17', 'Suction System', 'Inspection of the suction function', `${s16}`],
                ['18', 'Instrument channel system', 'Inspection of the instrument channel', `${s17}`],
                ['19', 'Auxiliary water feeding function', 'Inspection of the auxiliary water feeding function', `${s18}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'TGF-UC180J_' + sno}`; // Custom file name
