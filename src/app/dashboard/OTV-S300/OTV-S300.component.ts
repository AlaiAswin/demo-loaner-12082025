...
@Component({
  selector: 'app-OTV-S300',
  templateUrl: './OTV-S300.component.html',
  styleUrls: ['./OTV-S300.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Otvs300Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p)
        && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p)) {
        console.log('Pass')
        overRes = 'Pass'
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OTV-S300', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Clean and disinfect the OTV-S300 according to the instructions in the corresponding Instructions Manual.', `${s1}`],
              ...
              ['9', 'Checking the 3D Function', '3D test images are displayed on the 3D monitor.\nThe 3D monitor display is switched to 2D and 3D respectively when the video output from OTV-S300 is switched to 2D and 3D..', `${s9}`],
              ['10', 'Check of Communication with CLV-S200-IR', 'When the distance from the object changes between five to 60 mm in normal light observation, the automatic brightness adjustment works.\nLight source cable can be connected securely.', `${s10}`],
              ['11', 'Checking the Release Function', 'The internal memory access indicator and portable memory access indicator blink in orange during image saving, and then light up in green. \nImages saved in a portable memory can be displayed on the 3D monitor', `${s11}`],
              ['12', 'Custom switch', 'Confirm that the preset switches (the remote switches and the foot switches of the endoscope) work properly as assigned.', `${s12}`],
              ['13', 'Checking the Operation Sound', 'After the power switch is turned ON and any button on the touch panel is pressed, an operation sound is heard.', `${s13}`],
              ['14', 'Appearance check', 'There must be no excessive scratches, dirt, or discolored appearance.\nThe step and gap between the front panel and the top cover must be 1 mm or below. The external surfaces must be free of convex/ concave that physically makes cleaning difficult.\nThere must be no irregular burr or edges in appearance. Labeling must be appropriate.', `${s14}`],
              ['15', 'Turn off the power', 'Press the power switch of the OTV-S300.\nConfirm that the power of the OTV-S300 is turned off, and the power switch indicator goes out.', `${s15}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OTV-S300_' + sno}`; // Custom file name
