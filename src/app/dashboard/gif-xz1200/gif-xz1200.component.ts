...
(pdfMake as any).vfs = pdfFonts;
@Component({
  selector: 'app-gif-xz1200',
  templateUrl: './gif-xz1200.component.html',
  styleUrls: ['./gif-xz1200.component.css'],
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p) && (s17 == p) && (s18 == p) && (s19 == p) && (s20 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR gif-xz1200', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['10', 'Image system', 'Inspection of the endoscopic image', `${s10}`],
                ['11', 'Focus switching function -Setting the video system center', '1. When the NBI or RDI endoscopic image is displayed, switch to the WLI endoscopic image.\n2. Confirm which remote switch is assigned the focus switching function (it is assigned as the factory default setting of remote switch 5).', `${s11}`],
                ['12', 'Focus switching function -Inspection of switching to the near focus mode', '1. Press the remote switch to which the focus switching function is assigned.\n2. Confirm that the "Near Focus" indication is displayed in the upper right of the endoscopic image.\n3. Confirm that an object is clearly visible between about 2 and 4 mm from distal end of the endoscope.', `${s12}`],
                ['13', 'Focus switching function -Inspection of switching to the normal focus mode', '1. Press the remote switch to which the focus switching function is assigned.\n2. Confirm that the "Near Focus" indication in the upper right of the endoscopic image disappears.\n3. Confirm that an object at distance of about 20 mm from distal end of the endoscope is clearly visible.', `${s13}`],
                ['14', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s14}`],
                ['15', 'Air-feeding function -Confirmation of emitting no air bubbles', '1. Set the air supply pressure on the video system center to "High".\n2. Immerse the distal end of the endoscope in water to a depth of approximately 10 cm.\n3. Confirm that no air bubbles are emitted when the air/water valve is not operated. ', `${s15}`],
                ['16', 'Air-feeding function -Confirmation of emitting air bubbles', '1. Cover the hole in the air/water valve with your finger and confirm that air bubble are continuously emitted from the air/water nozzle.\n 2. Uncover the hole in the air/water valve and confirm that no air bubbles are emitted from air/water nozzle.', `${s16}`],
                ['17', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s17}`],
                ['18', 'Suction System', 'Inspection of the suction function', `${s18}`],
                ['19', 'Auxiliary water system', 'Inspection of the auxiliary water feeding function', `${s19}`],
                ['20', 'Instrument channel system', 'Inspection of the instrument channel ', `${s20}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'gif-xz1200_' + sno}`; // Custom file name
