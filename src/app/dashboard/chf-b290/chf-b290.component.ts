...
@Component({
  selector: 'app-chf-b290',
  templateUrl: './chf-b290.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class ChfB290Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p)) {
        console.log('Pass')
        overRes = 'Pass'
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR CHF-B290', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'CHF-B290_' + sno}`; // Custom file name
