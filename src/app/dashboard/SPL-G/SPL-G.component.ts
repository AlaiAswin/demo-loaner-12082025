...
@Component({
  selector: 'app-SPL-G',
  templateUrl: './SPL-G.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Splgcomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR SPL-G', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the SPL-G must be performed according to the Instructions Manual.', `${s1}`],
              ['2', 'Power ON', 'Verify power switch light comes on.\nBackground lights turns on.\nTransducer light ring is blinking.\nTransducer light ring quits blinking.\nStandard Power” light is illuminating green.\n “High Power” light is illuminating green', `${s2}`],
              ['3', 'Visual Inpsction', 'Verify each hand switch latches ultrasonic output when double clicked.\nVerify that the green footswitch light, lights up.\n Verify that the hand switches are disabled.\n “Standard Power” light is illuminating green', `${s3}`],
              ['4', 'Function check', 'Generator transducer receptacle light ring goes from blinking to solid when the transducer is plugged in.\nHand switch activates standard power and high power \nVerify each hand switch latches ultrasonic output when double clicked.', `${s4}`],
              ['5', 'Turn off the power', 'Press the power switch of the SPL-G.\nConfirm that the power of the SPL-G is turned off, and the power switch indicator goes out.', `${s5}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'SPL-G_' + sno}`; // Custom file name
