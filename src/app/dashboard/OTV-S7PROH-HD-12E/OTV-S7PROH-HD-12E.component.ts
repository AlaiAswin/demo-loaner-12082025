...
@Component({
  selector: 'app-OTV-S7PROH-HD-12E',
  templateUrl: './OTV-S7PROH-HD-12E.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Otvs7prohhd12ecomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OTV-S7PROH-HD-12E', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the OTV-S7PROH-HD-12E must be performed according to the Instructions Manual.', `${s1}`],
              ['2', 'Inspection of the camera head', 'Confirm that the camera head does not have the following damages.\nSurface of the camera head’s main body is free from cracks, burrs, or other damages.\nCamera cable is free from sagging, swelling, scratches, or holes.\nElectrical contacts of the video connector are free from bends or rust.\nWhen turning the endoscope mount lock counterclockwise with holding it lightly, confirm that the endoscope mount lock is free from looseness or backlash. \nWhen operating the endoscope mount, confirm that the endoscope mount is free from looseness or backlash', `${s2}`],
              ...
              ['4', 'Connection of the camera head and ancillary equipment (Connection to the endoscope)', 'Holding the camera head, gently touch the endoscope’s eyepiece to the endoscope mount while sliding the endoscope lock ring in the direction of the arrow. Then release the endoscope lock ring to secure the eyepiece in the specified position.', `${s4}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OTV-S7PROH-HD-12E_' + sno}`; // Custom file name
