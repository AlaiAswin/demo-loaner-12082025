...
@Component({
  selector: 'app-CLV-190',
  templateUrl: './CLV-190.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Clv190Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p)) {
        console.log('Pass')
        overRes = 'Pass'
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR CLV-190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning/disinfection', 'Clean and disinfect the CLV-190 according to the instructions described in the corresponding Instructions Manual.', `${s1}`],
              ...
              ['3', 'Cooling Fan', 'Place the hand over the ventilation on the back of the CLV-190 and verify that the air is exhausted from the ventilation for cooling.', `${s3}`],
              ['4', 'Lamp Life', 'Verify that lamp life is not expired.', `${s4}`],
              ['5', 'Examination light on ', 'No emergency lamp indicator on the front panel lights nor blinks.The examination light is emitted from the endoscope distal end.', `${s5}`],
              ['6', 'Examination light off', 'The examination lamp extinguishes and the lamp indicator switches to "STBY"(standby)', `${s6}`],
              ['7', 'Brightness mode selection function', 'Verify that the brightness mode indicator switches between "AUTO" and "MANUAL".', `${s7}`],
              ['8', 'Auto-Brightness', 'With the examination lamp illuminated, press the brightness mode button to switch to "AUTO" point the endoscope distal end towards an object. Move the endoscope distal end closer to the object and then away from it at the distance between 5 to 60 mm. Verify that the brightness of the endoscope image on the monitor is consistent.\n\nPress the brightness adjustment buttons "- " and "+" inspect the following items:\nEach time either of the brightness buttons is pressed a beep sounds and the brightness increase & decrease by 1 accordingly.\nWhen either of the buttons is pressed continuously, a beep sound and the brightness indicator changes in a continuous manner.\nThe brightness of the light emitted from the endoscope distal end increase ordecrease according to the brightness setting.', `${s8}`], ['9', 'Manual brightness ', 'With the examination lamp illuminated, press the brightness adjustment buttons "-" and "+" inspect the following items: \nEach time either of brightness buttons is pressed , a beep sounds and the brightness increase or decrease by 1 accordingly. \nWhen either of buttons is pressed continuously, a beep sounds and the brightness indicator changes in a continuous manner.\nThe brightness of light emitted from the endoscope distal end increase or decrease according to the brightness setting.', `${s9}`],
              ['10', 'Optical-Digital', 'Inspect the observation mode indicator for observation mode that is available for connected videoscope illuminates on the front panel.\nPress the optical digital observation mode button on the front panel. The observation mode indicator "ON" lights up. verify that display of the endoscope image switches to optical- digital observation mode indication with the observation mode indicator on the front panel.\nEach time the observation mode select button is pressed, optical-digital observation mode switches accordingly and the corresponding observation mode indicator lights up.', `${s10}`],
              ['11', 'Transillumination', 'The transiilumination indicator blinks. The examination light automatically changes to white light with the maximum light intensity for approximately 7 seconds.\nAfter 7 seconds transillumination function deactivated, the examination light returns to the original intensity. ', `${s11}`],
              ['12', 'Air/Water supply', 'The air flow indicator is "ON" lights up. Each time the air supply regulator button is pressed, the air supply regular indicator switches between L(low) ,M(medium) & H(high). ', `${s12}`],
              ['13', 'Exterior check', 'Inspect the CLV-190 exterior for noticeable scratches, grim, and/or deformation that may affect the functionality.', `${s13}`],
              ['14', 'Ventilation ', 'Inspect that the ventilation on the side and back of the CLV-190 is not clogged with dust and/or/ foreign objects.', `${s14}`],
              ['15', 'Power OFF ', 'Press the power switch and verify that the CLV-190 is turned off and the power indicator extinguishes.', `${s15}`]
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${' CLV-190_' + sno}`; // Custom file name
