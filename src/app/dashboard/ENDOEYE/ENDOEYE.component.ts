...
@Component({
  selector: 'app-ENDOEYE',
  templateUrl: './ENDOEYE.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Endoeyecomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR ENDOEYE', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              [{ text: 'S.N', fillColor: '#CCCCCC', alignment: 'center' }, { text: 'INSPECTION ITEM ', fillColor: '#CCCCCC', alignment: 'center' }, { text: 'INSPECTION STANDARD', fillColor: '#CCCCCC', alignment: 'center' }, { text: 'INSPECTION RESULT', alignment: 'center', fillColor: '#CCCCCC' }],
            ],
          }
        },

        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning/disinfection', 'Clean and disinfect the equipment according to the instructions described in the corresponding Instructions Manual. ', `${s1}`],
              ['2', 'Image system', '(a).Inspection of the endoscope image\n(b).White balance\n(c).NBI Image\n(d).3D Image check (If applicable)', `${s2}`],
              ['3', 'Endoscope connector', 'Connetion to the light source', `${s3}`],
              ['4', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s4}`],
              ['5', 'Overall apperance', '(a).Control unit check\n(b).Insertion tube\n(c).Video cable\n(d).Plug unit\n(e).Light guide connector\n(f).Labels on endoeye scope', `${s5}`],
      ...
      if (image_data_url) {
        content.unshift(
          { text: 'Issue date :29 Aug 2023 ', alignment: 'right', fontSize: 5, }, { text: 'REV NO : 00', alignment: 'right', fontSize: 5 },
          {
