...
@Component({
  selector: 'app-bf-1t60',
  templateUrl: './bf-1t60.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Bf1t60Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-1T60', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Image system', 'Inspection of the endoscopic image', `${s4}`],
                ['5', 'Suction System', 'Inspection of the suction function', `${s5}`],
                ['6', 'Instrument channel system', 'Inspection of the instrument channel', `${s6}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-1T60_' + sno}`; // Custom file name
