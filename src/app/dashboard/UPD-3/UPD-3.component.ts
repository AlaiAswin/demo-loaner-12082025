...
@Component({
  selector: 'app-UPD-3',
  templateUrl: './UPD-3.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Upd3Component {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR UPD-3', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Clean and disinfect the UPD-3 according to the instructions in the corresponding Instructions Manual.', `${s1}`],
              ['2', 'Exterior of UPD-3', 'Inspect the UPD-3 exterior for noticeable scratch marks ,grime and/or deformation that may affect the functionality. \nInspect the tube connected areas on the front panel for adherence of patient body fluid( e.g. blood) ', `${s2}`],
              ['3', 'Checking activation', 'When the power supply switch is pressed, the power is supplied and the LED of the front panel is lit.After activation, no abnormal sound or vibration is detected.Air is ventilated by fan.', `${s3}`],
              ['4', 'Conection detection', 'The reception antenna, external marker, reference plate, and scope can be detected', `${s4}`],
              ['5', 'Checking scope depiction', 'You can switch the IRIS level between HIGH and LOW by pressing the IRIS level change button on the operation panel.', `${s5}`],
              ['6', 'Checking marker depiction', 'The external marker SW functions properly.The spheres depiction is displyaed in gray.The external marker fault icon is not displayed.', `${s6}`],
              ['7', 'Checking the faults of the front panel', 'The front panel SW functions properly.', `${s7}`],
              ['8', 'Checking images', 'The monitor displays the image correctly. (XGA, SDI, Y/C)', `${s8}`],
              ['9', 'Power off', 'Press the power switch, verify that the UPD-3 is turned off and the power indicator extinguish.', `${s9}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'UPD-3_' + sno}`; // Custom file name
