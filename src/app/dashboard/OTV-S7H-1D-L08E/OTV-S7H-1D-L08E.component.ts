...
@Component({
  selector: 'app-OTV-S7H-1D-L08E',
  templateUrl: './OTV-S7H-1D-L08E.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Otvs7h1dl08ecomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OTV-S7H-1D-L08E', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the OTV-S7H-1D-L08E must be performed according to the Instructions Manual.', `${s1}`],
              ...
              ['4', 'Connection of the camera head and ancillary equipment (Connection to the endoscope)', 'Select an endoscope with a mount design that matches the camera head being used.\nHold the endoscope mount with one hand and press the endoscope lock button from both sides.\nPress the eyepiece of the endoscope gently into the endoscope mount and release the endoscope lock button.', `${s4}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OTV-S7H-1D-L08E_' + sno}`; // Custom file name
