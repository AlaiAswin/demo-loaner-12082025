...
@Component({
  selector: 'app-bf-mp160f',
  templateUrl: './bf-mp160f.component.html',
  styleUrls: ['./bf-mp160f.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class BfMp160fComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-MP160F', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4.1', 'Endoscope connector', 'Connection of ancillary equipment:\n1. Connection to the light source', `${s4a}`],
                ['4.2', 'Endoscope connector', 'Connection of ancillary equipment:\n2. Connection to the videoscope cable', `${s4b}`],
                ['5', 'Image system', 'Inspection of the endoscopic image', `${s5}`],
                ['6', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s6}`],
                ['7', 'Suction System', 'Inspection of the suction function', `${s7}`],
                ['8', 'Instrument channel system', 'Inspection of the instrument channel', `${s8}`],
                ['9', 'Water supply system', 'Inspection of the water feeding function', `${s9}`],
              ],
            }
          },
          {
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-MP160F_' + sno}`; // Custom file name
