...
@Component({
  selector: 'app-bf-uc180f',
  templateUrl: './bf-uc180f.component.html',
  styleUrls: ['./bf-uc180f.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class BfUc180fComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3a == p) && (s3b == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p)
          && (s11 == p) && (s12 == p) && (s13 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR BF-UC180F', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Endoscope connector', 'Connection to the light source', `${s4}`],
                ['5', 'Endoscope connector', 'Connection to the videoscope cable', `${s5}`],
                ['6', 'Endoscope connector;', 'Connection of the endoscope and ultrasonic cable', `${s6}`],
                ['7', 'Image system', 'Inspection of the endoscopic image', `${s7}`],
                ['8', 'Ultrasound imaging system', 'Inspection of the ultrasonic image with the compact endoscopic ultrasound center', `${s8}`],
                ['9', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s9}`],
                ['10', 'Water supply system', 'Inspection of the water feeding function', `${s10}`],
                ['11', 'Suction System', 'Inspection of the suction function', `${s11}`],
                ['12', 'Balloon pipeline system', 'Inspection of the water feeding function into the balloon', `${s12}`],
                ['13', 'Instrument channel system', 'Inspection of the instrument channel', `${s13}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'BF-UC180F_' + sno}`; // Custom file name
