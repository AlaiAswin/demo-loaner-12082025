...
@Component({
  selector: 'app-cf-q180al',
  templateUrl: './cf-q180al.component.html',
  styleUrls: ['./cf-q180al.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class CfQ180alComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder, private popupservice: PopUpService, private router: Router, private renderer: Renderer2, private location: Location) {
    this.formGroup = this.fb.group({
      ...
      s5a_result: [, Validators.required],
      s5b_result: [, Validators.required],
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        let s5a = data['s5a_result'];
        let s5b = data['s5b_result'];
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5a == p) && (s5b == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p)
          && (s11 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15a == p) && (s15b == p) && (s16 == p) && (s17 == p) && (s18 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR CF-Q180AL', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4', 'Insertion section', 'Inspection of the flexibility adjustment mechanism', `${s4}`],
                ['5.1', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation', `${s5a}`],
                ['5.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the UP/DOWN/RIGHT/LEFT angulation mechanism', `${s5b}`],
                ...
                ['17', 'Auxiliary water system', 'Inspection of the auxiliary water feeding function', `${s17}`],
                ['18', 'Instrument channel system', 'Inspection of the instrument channel', `${s18}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'CF-Q180AL_' + sno}`; // Custom file name
