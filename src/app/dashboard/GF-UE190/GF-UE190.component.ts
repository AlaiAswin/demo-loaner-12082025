...
(pdfMake as any).vfs = pdfFonts.vfs;
@Component({
  selector: 'app-GF-UE190',
  templateUrl: './GF-UE190.component.html',
  styleUrls: ['./GF-UE190.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Gfue190Component {
  formGroup: any;
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4 == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p) && (s12 == p) && (s13 == p) && (s14 == p) && (s15 == p) && (s16 == p) && (s17 == p) && (s18 == p) && (s19 == p) && (s20 == p) && (s21 == p)) {
          // console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          { text: 'DEMO-LOANER INSPECTION SHEET FOR GF-UE190', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
            ...
            table: {
              ...
              body: [
                ...
                ['4', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n1. Inspection for smooth operation\n2. Inspection of the UP/DOWN angulation mechanism', `${s4}`],
                ['5', 'Control section;\nElevator section', 'Inspection of the forceps elevator mechanism;\n1. Inspection for smooth operation', `${s5}`],
                ['6', 'Control section;\nSuction valve mounting section', 'Attaching the suction valve', `${s6}`],
                ['7', 'Control section;\nAir/water valve mounting section', 'Attaching the air/water valve', `${s7}`],
                ['8', 'Control section;\nBiopsy valve mounting section', 'Attaching the biopsy valve', `${s8}`],
                ['9', 'Endoscope connector', 'Connection to the light source', `${s9}`],
                ['10', 'Endoscope connector', 'Connection of the water container', `${s10}`],
                //['11', 'Endoscope connector', 'Connection of the videoscope cable', `${s11}`],
                ['11', 'Endoscope connector', 'Connection of the endoscope and ultrasonic cable', `${s12}`],
                ['12', 'Image system', 'Inspection of the endoscopic image', `${s13}`],
                ['13', 'Ultrasound imaging system', 'Inspection of the ultrasound image with the Olympus universal endoscopic ultrasound center EU-ME1', `${s14}`],
                ...(image_data_url !== null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
                ['14', 'Control section;\nSwitch system', 'Inspection of the remote switches', `${s15}`],
                ['15', 'Air supply system', 'Inspection of the air-feeding function', `${s16}`],
                ['16', 'Water supply system', 'Inspection of the objective lens cleaning function:\n1. Inspection of the water feeding function\n2. Inspection of removing the remaining water from the objective lens', `${s17}`],
                ['17', 'Balloon water supply system', 'Inspection of the water feeding function into the balloon', `${s18}`],
                ['18', 'Suction System', 'Inspection of the suction function', `${s19}`],
                ...(image_data_url === null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
                ['19', 'Balloon water suction system', 'Inspection of aspiration from the balloon water suction port', `${s20}`],

                ['20', 'Instrument channel system;\nforceps elevator system', 'Inspection of the instrument channel and forceps elevator', `${s21}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'GF-UE190_' + sno}`; // Custom file name
