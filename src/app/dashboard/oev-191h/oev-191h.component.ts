...
@Component({
  selector: 'app-oev-191h',
  templateUrl: './oev-191h.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Oev191hComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'DEMO-LOANER INSPECTION SHEET FOR OEV-191H', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ['1', 'Cleaning & Disinfection', 'Cleaning and disinfection of the OEV-191H must be performed according to the instructions manual.', `${s1}`],
              ['2', 'Power ON', 'Press the power switch on the OEV-191H.\nVerify that the power indicator on the monitor lights up green.', `${s2}`],
              ['3', 'Button operation', 'Press all buttons on the front panel of the OEV-191H.\nVerify that all buttons function normally', `${s3}`],
              ['4', 'Monitor screen', 'Connect the video system center to the OEV-191H with video cables (RGB,Svideo, Video) to transfer the color bar image.\nConnect the PC to the OEV-191H with the HD15 cable to transfer PC images.\nTurn ON the connected equipment.\nSelect each of the RGB,S-video, and Video signals for image display on the monitor.Inspect the following items:\nThe image is displayed on the monitor according to the selected signal.\nThere are no discoloration, noise, and distortion of the image displayed on the monitor.\nRepeat the above steps to inspect all the signals.\nInspect the LCD panel on the monitor for cracks.', `${s4}`],
              ['5', 'Output signal', 'Turn OFF the OEV-191H.\nConnect the output terminal and secondary monitor with video cables. The secondary monitor is equipped with terminals such as RGB, S-Video, Video,HD15.\nTurn ON both monitors.\nSet the same signal (RGB, S-Video, Video or HD15) for image display on both monitors. Inspect the following items:\nThe image on the OEV-191H is automatically transferred to the secondary monitor.\nThere are no discoloration, noise, and distortion of the image displayed on the secondary monitor.\nRepeat the above steps to inspect all the signals', `${s5}`],
              ['6', 'Power OFF', 'Press the power switch on the OEV-191H.\nVerify that the OEV-191H is turned off and the power indicator for the power switch extinguishes', `${s6}`],
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'OEV-191H_' + sno}`; // Custom file name
