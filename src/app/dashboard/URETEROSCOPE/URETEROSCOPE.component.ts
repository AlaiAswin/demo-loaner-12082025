...
@Component({
  selector: 'app-URETEROSCOPE',
  templateUrl: './URETEROSCOPE.component.html',
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class Ureteroscopecomponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      var content: Content[] = [
        ...
        { text: 'Asset Inspection Card - URETEROSCOPE', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
        {
          table: {
        ...
        { text: 'VISUAL INSPECTION OF THE ELECTRO SURGICAL GENERATOR AND ACCESSORIES', fontSize: 12 },
        {
          table: {
            widths: [25, 125, 275, 75],
            body: [

              ['1', 'PLANNED PERIODICAL SAFETY CHECK', 'NOT OVER DUE', `${s1}`],
              ['2', 'LABELS', 'ALL PRESENT & LEGIBLE', `${s2}`],
              ['3', 'INTEGRITY', 'PROCEDURE PASSED', `${s3}`],
              ['4', 'DISTAL END - DAMAGE/ABNORMALITIES ', 'NONE PRESENT', `${s4}`],
              ['5', 'INSERTION TUBE - DAMAGE/ABNORMALITIES', 'NONE PRESENT', `${s5}`],
              ['6', 'UNIVERSAL CABLE - DAMAGE/ABNORMALITIES', ' NONE PRESENT', `${s6}`],
              ['7', 'VIEO CONNECTOR - DAMAGE/ABNORMALITIES', ' NONE PRESENT', `${s7}`],
        ...
        { text: 'FUNCTION TEST', fontSize: 12 },

        {
          table: {
            ...
            body: [
              ...
              ['8', 'LIGHT TRANSMISSION', ' PROCEDURE PASSED', `${s8}`],
              ['9', 'CONNECTION', ' PROCEDURE PASSED', `${s9}`],
              ['10', 'BUTTONS', ' PROCEDURE PASSED', `${s10}`],
              ['11', 'IMAGE QUALITY(NON 3D)', ' PROCEDURE PASSED', `${s11}`],
              ['12', 'IMAGE QUALITY (3D)', ' PROCEDURE PASSED', `${s12}`],
            ],
          }
        },
      ...
      pdfDocGenerator.getBlob((blob) => {
        const fileName = `${'URETEROSCOPE_' + sno}`; // Custom file name
