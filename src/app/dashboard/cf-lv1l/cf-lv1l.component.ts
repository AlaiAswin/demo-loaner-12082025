...
@Component({
  selector: 'app-cf-lv1l',
  templateUrl: './cf-lv1l.component.html',
  styleUrls: ['./cf-lv1l.component.css'],
  imports: [FormsModule, ReactiveFormsModule, CommonModule],
  standalone: true
})
export class CfLv1lComponent {
  startTime: any;
  endTime: any;
  cdsDate: any;
  ...
  constructor(private fb: FormBuilder, private router: Router, private popupservice: PopUpService, private renderer: Renderer2, private location: Location) {
  ...
  async download() {
    ...
    if (this.formGroup.status == 'INVALID') {
      ...
      if (proceed == 'true') {
        ...
        if ((s1 == p) && (s2 == p) && (s3 == p) && (s4a == p) && (s4b == p) && (s5 == p) && (s6 == p) && (s7 == p) && (s8 == p) && (s9 == p) && (s10 == p)
          && (s11 == p) && (s12 == p) && (s13a == p) && (s13b == p) && (s14 == p) && (s15 == p)) {
          console.log('Pass')
          overRes = 'Pass'
        ...
        var content: Content[] = [
          ...
          { text: 'DEMO-LOANER INSPECTION SHEET FOR CF-LV1L', alignment: 'center', margin: [0, 10, 0, 5], bold: true, fontSize: 16 },
          {
            table: {
          ...
          },
          {
            table: {
              ...
              body: [
                ...
                ['4.2', 'Control section;\nAngle section', 'Inspection of the bending mechanisms:\n2. Inspection of the UP/DOWN angulation mechanism', `${s4b}`],
                ...
                ['8', 'Endoscope connector', 'Connection to the light source and video system', `${s8}`],
                ['9', 'Endoscope connector', 'Connection of the water container', `${s9}`],
                ['10', 'Image system', 'Inspection of the endoscopic image', `${s10}`],
                ['11', 'Control section;\nSwitch section', 'Inspection of the remote switches', `${s11}`],
                ['12', 'Air supply system', 'Inspection of the air-feeding function', `${s12}`],
                ...
                ['14', 'Suction System', 'Inspection of the suction function', `${s14}`],
                ...(image_data_url !== null ? [['', '', '', { text: '', pageBreak: 'before' }]] : []),
                ['15', 'Instrument channel system', 'Inspection of the instrument channel', `${s15}`],
        ...
        pdfDocGenerator.getBlob((blob) => {
          const fileName = `${'CF-LV1L_' + sno}`; // Custom file name
